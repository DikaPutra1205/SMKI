...
export function ThemeToggle() {
    ...
    return (
        ...
        >
            <Sun
                className={`absolute h-4.5 w-4.5 transition-all duration-200 ${isDark ? 'scale-0 rotate-90 opacity-0' : 'scale-100 rotate-0 opacity-100'}`}
            />
            <Moon
                className={`absolute h-4.5 w-4.5 transition-all duration-200 ${isDark ? 'scale-100 rotate-0 opacity-100' : 'scale-0 -rotate-90 opacity-0'}`}
            />
        </button>
    );
}
