    ...
    function submitForm(e: React.FormEvent) {
        ...
        if (modalMode === 'create') {
            ...
            form.transform((data) => ({
                ...data,
                framework_id: data.framework_id || null,
            })).patch(`/admin/kepatuhan/checklist-sessions/${editingId}`, {
                onSuccess: closeModal,
                preserveScroll: true,
            });
        }
                ...
                {can('checklist-session.create') && (
                    <>
                        <button
                            ...
                            className="border-border-strong text-navy bg-white hover:bg-surface inline-flex items-center gap-2 rounded-[10px] border px-4 py-2 text-xs font-semibold transition-colors sm:text-sm dark:border-slate-600 dark:bg-slate-900 dark:text-white dark:hover:bg-slate-800"
                        >
