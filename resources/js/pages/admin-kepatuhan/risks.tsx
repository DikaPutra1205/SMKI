        ...
        <AppLayout breadcrumbs={breadcrumbs} currentPath="/admin/kepatuhan/risks">
            ...
            <div className="page-head flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <p className="text-muted mt-1 text-xs sm:text-sm">{t('risks.subtitle')}</p>
                </div>

                <button
                    type="button"
                    disabled
                    className="bg-surface-2 border-border text-faint inline-flex items-center gap-2 rounded-[10px] border px-4 py-2 text-xs font-semibold sm:text-sm"
            ...
            <div className="grid grid-cols-2 gap-[14px] xl:grid-cols-4">
                {kpiCards.map((kpi) => (
                    <div key={kpi.label} className="border-border rounded-[14px] border bg-white p-[18px_20px] shadow-sm">
                        <div className="flex items-center justify-between">
                            <div className={`grid h-9 w-9 place-items-center rounded-[10px] ${kpi.iconClass}`}>
                                <kpi.Icon className="h-[18px] w-[18px]" />
                            </div>
                        </div>
                        <div className="text-navy mt-3 text-[26px] leading-none font-bold">{kpi.value}</div>
                        <div className="text-muted mt-1.5 text-xs font-semibold">{kpi.label}</div>
                    </div>
                ))}
            </div>
                                    ...
                                    <tr key={r.id} className="hover:bg-surface/50 transition-colors">
                                        ...
                                        <td className="px-5 py-4">
                                            ...
                                            {r.mitigation_plan || r.rencana_mitigasi ? (
                                                <div className="text-faint mt-0.5 line-clamp-1 text-xs">
                                                    {r.mitigation_plan || r.rencana_mitigasi}
                                                </div>
                                            ) : null}
                                        </td>
                                        ...
                                        <td className="text-body px-5 py-4 whitespace-nowrap">{r.risk_owner || r.pemilik_risiko || '—'}</td>
                                        <td className="px-5 py-4 whitespace-nowrap">
                                            <StatusBadge tone={STATUS_TONE[r.status] ?? 'gray'}>
                                                {t(`status.${r.status as 'open' | 'mitigated' | 'accepted'}`)}
                                            </StatusBadge>
                                        </td>
                                        <td className="text-faint px-5 py-4 whitespace-nowrap">{t('risks.noTarget')}</td>
                                    </tr>
                                ))
                            ) : (
                                <tr>
                ...
                {detailTarget && (
                    <div className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-4">
                            ...
                            <div>
                                <h4 className="text-navy text-sm font-bold">{t('risks.status')}</h4>
                                <div className="mt-2">
