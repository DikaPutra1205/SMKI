...
export default function Units({ units }: Props) {
    ...
    const blockedIds = useMemo(() => (editingId ? descendantsWithSelf(units, editingId) : new Set<number>()), [units, editingId]);
    const parentOptions = useMemo(() => units.filter((u) => !blockedIds.has(u.id)), [units, blockedIds]);

    const breadcrumbs = [{ label: t('common.dashboard'), href: '/admin/superadmin/dashboard' }, { label: 'Manajemen Unit' }];
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs} currentPath="/admin/superadmin/units">
            ...
            <ConfirmDialog
                ...
                description={delTarget ? `Apakah Anda yakin ingin menghapus unit "${delTarget.nama}"? Tindakan ini tidak dapat dibatalkan.` : ''}
