    ...
    return (
        <>
            ...
            <div className="flex min-h-screen flex-col bg-white dark:bg-slate-900 lg:flex-row">
                ...
                <main className="bg-surface dark:bg-slate-900 flex flex-1 items-center justify-center px-6 py-10">
                    <div className="w-full max-w-md">
                        <div className="border-border dark:border-slate-700 rounded-[18px] border bg-white dark:bg-slate-900 p-8 shadow-md">
                            <div className="mb-6 flex items-center gap-3 lg:hidden">
                                ...
                                <div>
                                    <strong className="text-navy dark:text-white block text-[15px] font-bold">{t('layout.brand')}</strong>
                                    <span className="text-faint dark:text-slate-500 block text-[11px] tracking-[.12em] uppercase">{t('layout.suite')}</span>
                                </div>
                            </div>
                            ...
                            <h1 className="text-navy dark:text-white text-xl font-bold">{t('auth.forgot.title')}</h1>
                            <p className="text-muted dark:text-slate-400 mt-1 text-sm">{t('auth.forgot.subtitle')}</p>

                            {sent && (
                                <p className="border-success-border dark:border-emerald-800 bg-success-bg text-success dark:text-emerald-400 mt-4 rounded-[10px] border px-3 py-2 text-sm font-medium">
                                    {t('auth.resetSent')}
                                </p>
                            )}
                            ...
                            <form onSubmit={submit} className="mt-6 flex flex-col gap-4">
                                <div>
                                    ...
                                    <div className="relative">
                                        <Mail className="text-faint dark:text-slate-500 absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2" />
                                        <input
                                            id="email"
                                            type="email"
                                            value={data.email}
                                            ...
                                            autoFocus
                                            className="border-border-strong dark:border-slate-600 text-ink dark:text-white placeholder:text-faint dark:placeholder:text-slate-500 focus:border-primary focus:ring-primary/20 h-11 w-full rounded-[10px] border bg-white dark:bg-slate-900 pr-3 pl-10 text-sm focus:ring-2 focus:outline-none"
                                        />
                                    </div>
                                    {errors.email && <p className="text-danger dark:text-red-400 mt-1 text-xs font-medium">{errors.email}</p>}
                                </div>
                                ...
                                <Link href={route('login')} className="text-muted dark:text-slate-400 hover:text-navy dark:hover:text-white text-center text-sm font-medium">
