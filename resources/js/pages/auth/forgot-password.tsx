    ...
    return (
        <>
            ...
            <div className="flex min-h-screen flex-col bg-white lg:flex-row dark:bg-slate-900">
                ...
                <main className="bg-surface flex flex-1 items-center justify-center px-6 py-10 dark:bg-slate-900">
                    <div className="w-full max-w-md">
                        <div className="border-border rounded-[18px] border bg-white p-8 shadow-md dark:border-slate-700 dark:bg-slate-900">
                            <div className="mb-6 flex items-center gap-3 lg:hidden">
                                ...
                                <div>
                                    <strong className="text-navy block text-[15px] font-bold dark:text-white">{t('layout.brand')}</strong>
                            ...
                            <h1 className="text-navy text-xl font-bold dark:text-white">{t('auth.forgot.title')}</h1>
                            <p className="text-muted mt-1 text-sm dark:text-slate-400">{t('auth.forgot.subtitle')}</p>

                            {sent && (
                                <p className="border-success-border bg-success-bg text-success mt-4 rounded-[10px] border px-3 py-2 text-sm font-medium dark:border-emerald-800 dark:text-emerald-400">
                                    {t('auth.resetSent')}
                                </p>
                            )}
                            ...
                            <form onSubmit={submit} className="mt-6 flex flex-col gap-4">
                                <div>
                                    ...
                                    <div className="relative">
                                        <Mail className="text-faint absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 dark:text-slate-500" />
                                        <input
                                            id="email"
                                            type="email"
                                            value={data.email}
                                            ...
                                            autoFocus
                                            className="border-border-strong text-ink placeholder:text-faint focus:border-primary focus:ring-primary/20 h-11 w-full rounded-[10px] border bg-white pr-3 pl-10 text-sm focus:ring-2 focus:outline-none dark:border-slate-600 dark:bg-slate-900 dark:text-white dark:placeholder:text-slate-500"
                                        />
                                    </div>
                                    {errors.email && <p className="text-danger mt-1 text-xs font-medium dark:text-red-400">{errors.email}</p>}
                                </div>
                                ...
                                <Link
                                    href={route('login')}
                                    className="text-muted hover:text-navy text-center text-sm font-medium dark:text-slate-400 dark:hover:text-white"
                                >
